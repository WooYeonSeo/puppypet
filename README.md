...
- **Claude Code 워처** — `~/.claude/projects/**/*.jsonl`과 `~/.claude/sessions/*.json`을 감시해
  - 작업 중일 때 강아지 머리 위에 **💭** 표시
  - 권한 프롬프트(예: Bash 승인 대기) 중일 때는 **🔔** 표시
  - 턴이 끝나면 _"…클코 작업 끝났다멍"_ 말풍선 → 탭하면 마지막 응답 본문 팝오버 (사용자가 닫기 전까지 유지)

<p align="center">
  <img src="docs/thinking.png" alt="Claude Code가 작업 중일 때 강아지 머리 위에 뜨는 💭" width="380">
  <br>
  <sub>Claude Code가 작업 중일 때는 강아지 머리 위에 <b>💭</b>가 떠서 알려줍니다.</sub>
</p>
